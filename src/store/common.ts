...
export const handleDirectory: any = async (
  handle: any,
  parentName?: string,
  toJSON?: boolean
) => {
  ...
  for await (const directory of handle) {
    ...
    if (directory[1].kind === "directory") {
      ...
      list.push({
        ...
        parentName: name,
        children: await handleDirectory(directory[1]!.entries(), name, toJSON),
      });
    }
    else {
      ...
      else obj.config = JSON.parse(await file!.text());
      list.push(obj);
    }
  }
  return list; // 返回处理后的目录列表
}
